...
const reminderQueue = computed(() => {
  ...
  invoicesDueSoon.value.forEach((invoice) => {
    const applicableTemplates = (templates.value || []).filter(
      (t) => t.enabled && t.offsetDays <= 0
    );
...
// Get dropdown menu items for template
const getTemplateMenuItems = (template: any) => {
  return [
    [
      {
        label: 'Edit',
  ...
  try {
    if (editingTemplate.value) {
      ...
      toast.add({
        ...
        description:
          "Your new reminder template has been created successfully.",
        color: "emerald",
      });
    }
...
const sendTestReminders = async () => {
  ...
  try {
    const result = await $fetch("/api/reminders/send", {
      method: "POST",
      body: {
        test: true,
      },
    ...
    toast.add({
      title: "Test reminders processed",
      description: `${result.sent} reminder(s) would be sent. Check console for details.`,
      color: "info",
    });
  } catch (error) {
    toast.add({
...
const checkReminders = async () => {
  isSendingReminders.value = true;
  try {
    const result = await $fetch("/api/reminders/check");

    toast.add({
      title: "Reminders checked",
      description: `${result.sent} reminder(s) sent successfully.`,
      color: "success",
    });
  } catch (error) {
    toast.add({
      title: "Error checking reminders",
      description: error instanceof Error ? error.message : "Please try again.",
      color: "red",
    });
  } finally {
...
<template>
  <div
    class="min-h-screen bg-gradient-to-br from-slate-50 to-amber-50/30 pb-24"
  >
    <!-- Header Section -->
    <div
        ...
        >
          <div class="space-y-3">
            ...
            <p class="text-sm text-slate-600 leading-relaxed">
              Set up automated WhatsApp, SMS, and email reminders that send at
              the perfect time to get you paid faster.
            </p>
          </div>
    ...
    <div class="grid gap-8 xl:grid-cols-[1.5fr,1fr]">
      ...
      <div class="space-y-6">
        ...
        >
          <div
            class="bg-gradient-to-r from-amber-50/80 to-orange-50/60 px-6 py-4 border-b border-slate-200/60"
          >
            <div class="flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-500/25"
              >
                <svg
              ...
              <div>
                ...
                <p class="text-sm text-slate-600">
                  Configure when and how to remind clients
                </p>
              </div>
            </div>
          </div>
          <div class="p-6">
              ...
              >
                      ...
                      <div class="flex flex-wrap items-center gap-3 text-sm">
                        <span class="flex items-center gap-1.5">
                          ...
                          <span class="text-slate-600">{{
                            formatOffset(template.offsetDays)
                          }}</span>
                        </span>
                      </div>
                    </div>
                ...
                <div class="flex justify-end">
                  <USwitch
                    ...
                    :ui="{
                      wrapper: 'relative inline-flex h-5 w-9 flex-shrink-0',
            ...
            <div v-else class="text-center py-12">
              <div
                class="w-16 h-16 rounded-full bg-amber-100 mx-auto mb-4 flex items-center justify-center"
              >
                <svg
                ...
                >
                  <path
                    d="M10,21H14A2,2 0 0,1 12,23A2,2 0 0,1 10,21M21,19V20H3V19L5,17V11C5,7.9 7.03,5.17 10,4.29C10,4.19 10,4.1 10,4A2,2 0 0,1 12,2A2,2 0 0,1 14,4C14,4.1 14,4.19 14,4.29C16.97,5.17 19,7.9 19,11V17L21,19M17,11A5,5 0 0,0 12,6A5,5 0 0,0 7,11V18H17V11Z"
                  />
                </svg>
              </div>
              ...
              <p class="text-sm text-slate-600 mb-6 max-w-sm mx-auto">
                Create your first reminder template to start automating payment
                follow-ups.
              </p>
              <UButton
                color="amber"
      ...
      <div class="space-y-6">
        ...
        >
          <div
            class="bg-gradient-to-r from-blue-50/80 to-indigo-50/60 px-6 py-4 border-b border-slate-200/60"
          >
            <div class="flex items-center gap-3">
              <div
          ...
          <div class="p-6">
                    ...
                    <div class="flex items-center gap-3 text-xs text-slate-500">
                      ...
                      <span class="flex items-center gap-1">
                        <UIcon
                          ...
                          :class="`w-3.5 h-3.5 text-${getChannelColor(
                            item.channel
                          )}-500`"
                        />
            ...
            <div v-else class="text-center py-8">
              <div
                class="w-12 h-12 rounded-full bg-blue-100 mx-auto mb-3 flex items-center justify-center"
              >
                <svg
        ...
        <!-- Info Card -->
        <div
          class="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200/60 rounded-xl p-6"
        >
            ...
            <div>
              ...
              <ul class="space-y-2 text-xs text-purple-700">
                <li class="flex items-start gap-2">
                  ...
                  <span
                    >Templates define when to send reminders relative to invoice
                    due dates</span
                  >
                </li>
                <li class="flex items-start gap-2">
                  <span class="text-purple-500 mt-0.5">•</span>
                  <span
                    >WhatsApp reminders go out first for instant delivery</span
                  >
                </li>
                <li class="flex items-start gap-2">
                  <span class="text-purple-500 mt-0.5">•</span>
          ...
          <p
            class="text-xs text-purple-600 bg-purple-100/50 rounded-lg px-3 py-2"
          >
            🚀 Integration with WhatsApp Business API and Africa's Talking
            coming soon
          </p>
        </div>
      </div>
        ...
        <form @submit.prevent="saveTemplate" class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-slate-500">
              Give your reminder template a descriptive name
            </p>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
              ...
              <span class="text-sm text-slate-600"
                >days
                {{ templateForm.offsetDays >= 0 ? "after" : "before" }} due
                date</span
              >
            </div>
          </div>
          ...
          >
            <div class="flex-1">
              ...
              <p class="text-xs text-slate-500 mt-1">
                Template will automatically send reminders when enabled
              </p>
            </div>
            <USwitch
    ...
    >
      <template #body>
        <div class="space-y-4">
          <p class="text-sm text-slate-700">
            ...
            <span v-if="templateToDelete" class="font-semibold"
              >"{{ templateToDelete.label }}"</span
            >?
          </p>
          <div class="rounded-lg bg-red-50 border border-red-200 p-3">
            <p class="text-xs text-red-700">
              This will remove the template permanently. Any scheduled reminders
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          <UButton color="red" @click="deleteTemplate">
            Delete Template
          </UButton>
        </div>
      </template>
