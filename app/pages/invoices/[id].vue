...
const issueDate = computed(() =>
  invoice.value ? formatDate(invoice.value.issueDate) : undefined
);
...
const amountDue = computed(() =>
  totals.value && invoice.value
    ? formatCurrency(
        ...
        invoice.value.currency ?? business.value?.currency ?? "GHS"
      )
    : undefined
);
...
const launchPaystackPayment = async () => {
  ...
  try {
    const response = await $fetch<{
      authorizationUrl: string;
      reference: string;
    }>(`/api/public/invoices/${invoiceId.value}/paystack`, { method: "POST" });

    const url =
      response.authorizationUrl || data.value?.paystack?.authorizationUrl;
    if (!url) {
    ...
    const providerMessage = (err as { data?: { providerMessage?: string } })
    ...
    <div class="mx-auto flex w-full max-w-4xl flex-col gap-6">
      ...
      <div v-else-if="error" class="rounded-3xl bg-white p-8 shadow-sm">
        ...
        <p class="mt-3 text-sm text-slate-600">
          We could not find this invoice. Please check the link you were given
          or contact the business owner for a fresh link.
        </p>
      ...
      <div v-else-if="invoice && business" class="flex flex-col gap-6">
        <div class="rounded-3xl bg-white p-6 shadow-sm">
          ...
          >
            <div>
              ...
              <p class="mt-4 text-sm text-slate-500">
                {{ business.name }}
                <span v-if="business.address" class="text-slate-400">
            ...
            <div class="flex flex-col items-start gap-2 sm:items-end">
              ...
              <div
                ...
                v-if="business.phone || business.email"
              >
          ...
          <div class="mt-6 grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
            <div>
              ...
              <p v-if="client?.businessName" class="text-slate-500">
                {{ client.businessName }}
              </p>
              <p v-if="client?.email" class="mt-1 text-slate-500">
                {{ client.email }}
              </p>
              <p v-if="client?.phone" class="text-slate-500">
                {{ client.phone }}
              </p>
            </div>
            <div>
        ...
        <div class="rounded-3xl bg-white p-6 shadow-sm">
          ...
          <p v-if="!isPaid" class="mt-2 text-sm text-slate-600">
            Use the button below to open a secure Paystack checkout and complete
            your payment by mobile money or card. Once Paystack confirms your
            transfer, the invoice will automatically update.
          </p>
          <p v-else class="mt-2 text-sm text-emerald-600">
            This invoice has already been marked as paid. You can keep the
          ...
          <p class="mt-4 text-xs text-slate-500">
            ...
            <span v-if="business.whatsappNumber"
              >WhatsApp {{ business.whatsappNumber }}</span
            >
            <span v-if="business.phone && !business.whatsappNumber"
              >phone {{ business.phone }}</span
            >
            <span v-if="business.email">
              <span v-if="business.whatsappNumber || business.phone"> or </span
              >{{ business.email }}
            </span>
            .
          </p>
